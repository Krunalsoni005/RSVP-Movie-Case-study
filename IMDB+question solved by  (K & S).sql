...
--- in movie table country, worlwide_gross_income , languages , production company have null values 
...
-- movies relesed on  month wise 

select month(date_published) as num_month, count(title) as  num_movies
from movie
...
select count(id) as num_movies , year
from movie
...
select genre, year, count(id) as num_movies
from movie as m
...
select genre, avg(duration) as avg_durations
from movie as m
inner join genre as g
on m.id=g.movie_id
group by genre
...
/* Output format:
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+
| min_avg_rating|	maxratings_avg_rating	|	min_total_votes   |	max_total_votes 	 |min_median_rating|min_median_rating|
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+
|		0		|			5		|	       177		  |	   2000	    		 |		0	       |	8			 |
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+*/
-- Type your code below:
...
select median_rating, count(movie_id) as movie_count_number
from movie as m
inner join ratings as r
on  m.id = r.movie_id
...
Let’s find out the top three directors in the top three genres who can be hired by RSVP Movies.*/
 ...
 -- Q19. Who are the topdirector_mapping three directors in the top three genres whose movies have an average rating > 8?
...
select title,
       case
             when avg_rating >8 then 'super hits'
...
SELECT d.name_id, name, d.movie_id,
	   m.date_published, 
       LEAD(date_published, 1) OVER(ORDER BY date_published, d.movie_id) AS next_movie_date
